defmodule ChatServer do
  ...
  def init(:ok) do
    ...
    salas = if map_size(salas) == 0 do
      # Crear sala predeterminada "general"
      %{"general" => %{creador: "sistema", miembros: []}}
    else
      salas
    end
  ...
  defp broadcast_mensaje_sistema(mensaje, estado) do
    Enum.each(estado.usuarios_conectados, fn {_, {pid, nodo}} ->
      try do
        # Usar :erlang.send para envío entre nodos
        :erlang.send({pid, nodo}, {:mensaje_sistema, mensaje})
      rescue
      ...
      Enum.each(estado.salas[nombre_sala].miembros, fn miembro ->
        case Map.get(estado.usuarios_conectados, miembro) do
          {pid, nodo} ->
            try do
              # Usar :erlang.send para envío entre nodos
              :erlang.send({pid, nodo}, {:mensaje_sistema, "[#{nombre_sala}] #{mensaje}"})
            rescue
              _ -> :ok  # Ignorar errores de envío
            end
          _ -> :ok
